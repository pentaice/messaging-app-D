...
const HomeScreen = ({ navigation }) => {
  ...
  const { conversations, startConversation } = useContext(ChatContext);
    ...
    navigation.setOptions({
      headerRight: () => (
        <View style={styles.headerRight}>
          <Text style={styles.userCodeText}>Kodunuz: {currentUser?.uid.substring(0, 8)}</Text>
          <TouchableOpacity 
  ...
  const handleStartConversation = async () => {
    if (userCode.length >= 6) {
      await startConversation(userCode, messageMode);
      setModalVisible(false);
      setUserCode('');
      navigation.navigate('Chat', { userId: userCode });
    }
  };
  ...
  const renderConversationItem = ({ item }) => (
    ...
    >
      <View style={styles.avatarContainer}>
        <Text style={styles.avatarText}>{item.userName?.charAt(0) || item.userId.charAt(0)}</Text>
      </View>
      <View style={styles.conversationDetails}>
        <View style={styles.conversationHeader}>
          <Text style={styles.userName}>{item.userName || item.userId.substring(0, 8)}</Text>
